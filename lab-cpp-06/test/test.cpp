...
class FindDisciplinesTest : public ::testing::Test {
protected:
    void SetUp() override {
        disciplines.push_back(&ds1);
        disciplines.push_back(&ds2);
...
TEST_F(FindDisciplinesTest, ElementsExist) {
    std::vector<Discipline*> result = findDisciplines(disciplines, "hours", "5");
    ASSERT_EQ(result.size(), 1);
    ASSERT_EQ(result[0]->getHours(), 5);
}

TEST_F(FindDisciplinesTest, ElementsDoNotExist) {
    std::vector<Discipline*> result = findDisciplines(disciplines, "name", "Танцы_с_бубном");
